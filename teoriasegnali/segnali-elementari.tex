...
%	\item[Segnale analogico] rappresentato da una funzione reale e continua: $s(t) \in \Re$, $t \in \Re$.
%	\item[Segnale digitale o numerico] rappresentato da una funzione tempo-discreta definita in un insieme discreto: $s(n) \in Z$, $n \in Z$.
...
Se $P_s<+\infty$, $s$ si definisce \textsc{segnale a potenza finita} oppure \textsc{segnale di potenza}\index{segnale!di potenza}.
...
% Se $t_0>0$ il segnale sarà ritardato, se $t_0<0$ il segnale sarà anticipato.
...
Il segnale è discontinuo in $\pm\frac{T}{2}$ ma si può estendere per continuità definendo
$s(t_0)=\frac{1}{2}[s(t_0^-)+s(t_0^+)]$
...
$\tau=\frac{T}{2}$ corrsponde al tempo in cui il segnale è diverso da zero, inoltre il rapporto $\frac{\tau}{T}$ è chiamato \emph{duty cycle}.
Se $\tau=\frac{T}{2}$ il \emph{duty cycle} $\frac{\tau}{T}$ è del 50\%, con un valor medio $1/2$.
...
\end{equation}
Il seno cardinale è un segnale a zeri equidistanti.
